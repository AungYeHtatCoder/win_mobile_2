   ...
   <div class="row">
    ...
    <div class="col-xl-12">
     ...
     <h1 class="page-header">
      Ordered Products for {{ $order->user->name }}
     </h1>
      ...
      <div class="card">
       <div class="card-body">
        <table id="datatableDefault" class="table text-nowrap w-100">
         ...
         <tbody>
          @foreach($orderProducts as $key => $productOrder)
          <tr>
           <td>{{ ++$key }}</td>
           <td>{{ $productOrder->product->name ?? $productOrder->accessory->name }}</td>
           <td>{{ $productOrder->color->name }}</td>
           <td>{{ $productOrder->product_prices->storage->name ?? "" }}</td>
           <td>{{ $productOrder->product_prices->ram->name ?? "" }}</td>
           <td>{{ $productOrder->qty }} pcs</td>
           <td>{{ number_format($productOrder->unit_price) }} MMK</td>
           <td>{{ number_format($productOrder->total_price) }} MMK</td>
           <td>{{ $productOrder->created_at->format('F j, Y') }}</td>
           <td>{{ $productOrder->updated_at->format('F j, Y') }}</td>
           <td>
            {{-- <a href="{{ url('/productOrders/pending/') }}" class="btn btn-primary btn-sm">Edit</a> --}}
            {{-- <a href="{{ url('/productOrder_detail/'.$productOrder->id) }}" class="btn btn-info btn-sm">Show</a> --}}
